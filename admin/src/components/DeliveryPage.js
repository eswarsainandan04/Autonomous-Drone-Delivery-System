...
const DeliveryPage = () => {
  ...
  // Load selected warehouse from sessionStorage on component mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (selectedWarehouse) {
      ...
      setDeliveryMissions([])
      // Clear selected warehouse from sessionStorage if no warehouse is selected
      sessionStorage.removeItem("selectedWarehouse");
    }
  }, [selectedWarehouse, droneAssignments])
  ...
  const handleMonitorClick = (mission) => {
    navigate("/monitor", {
      state: {
        packageData: mission,
        // Pass the selected warehouse data to Monitor.js
  ...
  const clearWarehouseSelection = () => {
    ...
    setShowWarehouseDropdown(false)
    // Clear sessionStorage when warehouse selection is cleared
    sessionStorage.removeItem("selectedWarehouse")
  }
  ...
  const filteredMissions = deliveryMissions.filter((mission) => {
    const matchesGeneralSearch =
      ...
      mission.destination?.toLowerCase().includes(missionSearchTerm.toLowerCase()) ||
    ...
    const matchesDestination =
      ...
      mission.destination?.toLowerCase().includes(missionDestinationFilter.toLowerCase())
