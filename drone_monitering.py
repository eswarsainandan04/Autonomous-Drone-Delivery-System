...
def get_drone_monitoring_data(drone_id):
    """
    API endpoint to get comprehensive drone monitoring data with source and destination coordinates
    """
    try:
        ...
        # Get drone data with source and destination coordinates
        cursor.execute("""
            SELECT *, source_lat, source_lng, dest_lat, dest_lng 
            FROM dronesdata 
            WHERE drone_id = %s
        """, (drone_id,))
        ...
        # Get destination data from packagemanagement (if exists)
        cursor.execute("""
        ...
        # Get warehouse coordinates (if warehouse_name exists in package data)
        warehouse_coords = None
        if package_data and package_data['warehouse_name']:
            cursor.execute("""
                SELECT latitude, longitude, name
                FROM warehouses
                WHERE name = %s
            """, (package_data['warehouse_name'],))
            warehouse_coords = cursor.fetchone()
        
        cursor.close()
        conn.close()
        
        # Prepare source coordinates (from dronesdata table)
        source_coords = None
        if drone_data['source_lat'] and drone_data['source_lng']:
            source_coords = {
                'latitude': float(drone_data['source_lat']),
                'longitude': float(drone_data['source_lng'])
            }
        
        # Prepare destination coordinates (from dronesdata table)
        destination_coords = None
        if drone_data['dest_lat'] and drone_data['dest_lng']:
            destination_coords = {
                'latitude': float(drone_data['dest_lat']),
                'longitude': float(drone_data['dest_lng'])
            }
        ...
        response_data = {
            ...
            "package": dict(package_data) if package_data else None,
...
def get_drone_status(drone_id):
    ...
    try:
        ...
        # Get current drone status with source and destination coordinates
        cursor.execute("""
            SELECT drone_id, drone_name, last_known_lat, last_known_lng, 
                   battery_capacity, status, source_lat, source_lng, dest_lat, dest_lng
            FROM dronesdata 
            WHERE drone_id = %s
        """, (drone_id,))
...
if __name__ == '__main__':
    print("Starting Drone Monitoring Server...")
    print(f"Database: {DB_HOST}:{DB_PORT}/{DB_NAME}")
    print("Server will be available at: http://localhost:5095")
