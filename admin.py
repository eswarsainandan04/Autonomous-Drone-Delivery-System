...
DB_NAME = "shadowfly"
DB_USER = "postgres"
DB_PASS = "admin" # Consider using environment variables for passwords
DB_PORT = 5432
...
def create_tables():
    ...
    try:
        ...
        # Create ddts table with updated schema
        cur.execute("""
            CREATE TABLE IF NOT EXISTS ddts (
                ...
                total_racks INTEGER,
                control_key CHARACTER VARYING(225)
            );
        """)
        app.logger.info("Table 'ddts' ensured to exist with updated schema.")
...
def add_ddt():
    ...
    control_key = data.get('control_key')  # New field
    ...
    try:
        ...
        sql = """
            ...
            VALUES (%s, %s, %s, %s, NULL, NULL, NULL, NULL, NULL, NULL, %s, %s)
            RETURNING id, name, latitude, longitude, status, total_racks, control_key
            """
        values = (name, lat_float, lon_float, status, total_racks_int, control_key)
...
def update_ddt(ddt_id):
    ...
    app.logger.info(f"[DEBUG][UPDATE DDT ID: {ddt_id}] Extracted name: '{name}', status: '{status}', latitude: '{latitude}', longitude: '{longitude}', total_racks: '{total_racks}', control_key: '{control_key}'")

    if name is None and status is None and latitude is None and longitude is None and total_racks is None and control_key is None:
    ...
    if total_racks is not None:
        try:
            total_racks_int = int(total_racks)
            if total_racks_int < 0 or total_racks_int > 6:
                app.logger.error(f"[DEBUG][UPDATE DDT ID: {ddt_id}] Invalid total_racks value: {total_racks}")
                return jsonify({'error': 'Total racks must be a number between 0 and 6.'}), 400
            update_fields.append("total_racks = %s")
            update_values.append(total_racks_int)
        except ValueError:
            app.logger.error(f"[DEBUG][UPDATE DDT ID: {ddt_id}] Invalid total_racks value: {total_racks}")
            return jsonify({'error': 'Invalid total_racks value. Must be a number.'}), 400
    if control_key is not None:
        update_fields.append("control_key = %s")
        update_values.append(control_key)
    ...
    try:
        ...
        sql = f"UPDATE ddts SET {', '.join(update_fields)} WHERE id = %s RETURNING id, name, latitude, longitude, status, total_racks, control_key"
...
# New endpoint to get DDT details by name
@app.route('/get_ddt_by_name/<string:ddt_name>', methods=['GET'])
def get_ddt_by_name(ddt_name):
    app.logger.info(f"[DEBUG][GET DDT BY NAME] For DDT: {ddt_name}")
    conn = None
    cur = None
    try:
        conn = get_db_connection()
        ...
        cur.execute("""
            SELECT * FROM ddts WHERE name = %s
        """, (ddt_name,))
        ddt_record = cur.fetchone()
        
        if ddt_record:
            ddt_item = dict(ddt_record)
            if ddt_item.get('latitude'):
                ddt_item['latitude'] = float(ddt_item['latitude'])
            if ddt_item.get('longitude'):
                ddt_item['longitude'] = float(ddt_item['longitude'])
            return jsonify(ddt_item), 200
        else:
            return jsonify({'error': f'DDT with name {ddt_name} not found'}), 404
            
    except (Exception, psycopg2.Error) as e:
        app.logger.error(f"[DEBUG][GET DDT BY NAME] Error for {ddt_name}: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500
    finally:
        if cur: cur.close()
        if conn: conn.close()

# New endpoint to get packages by rack IDs
@app.route('/get_packages_by_racks', methods=['POST'])
def get_packages_by_racks():
    data = request.json
    ...
    valid_rack_ids = [rack_id for rack_id in rack_ids if rack_id is not None and rack_id != '']
    
    if not valid_rack_ids:
        return jsonify([]), 200
    
    app.logger.info(f"[DEBUG][GET PACKAGES BY RACKS] For rack IDs: {valid_rack_ids}")
    conn = None
    cur = None
    try:
        conn = get_db_connection()
        ...
        placeholders = ','.join(['%s'] * len(valid_rack_ids))
        sql = f"""
        ...
        cur.execute(sql, valid_rack_ids)
        package_records = cur.fetchall()
        
        result_list = []
        for row in package_records:
            package_item = dict(row)
            # Convert any numeric fields to appropriate types
            if package_item.get('destination_lat'):
                package_item['destination_lat'] = float(package_item['destination_lat'])
            if package_item.get('destination_lng'):
                package_item['destination_lng'] = float(package_item['destination_lng'])
            if package_item.get('last_known_lat'):
                package_item['last_known_lat'] = float(package_item['last_known_lat'])
            if package_item.get('last_known_lng'):
                package_item['last_known_lng'] = float(package_item['last_known_lng'])
            result_list.append(package_item)
        
        return jsonify(result_list), 200
        
    except (Exception, psycopg2.Error) as e:
        app.logger.error(f"[DEBUG][GET PACKAGES BY RACKS] Error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500
    finally:
        if cur: cur.close()
        if conn: conn.close()
