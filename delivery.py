...
def get_package_monitor_data(package_id):
    ...
    try:
        ...
        # Get source coordinates from dronesdata table using assigned_drone_id
        cur.execute("""
            SELECT source_lat, source_lng 
            FROM dronesdata 
            WHERE drone_id = %s;
        """, (package_row[10],))  # assigned_drone_id is at index 10
        
        drone_source_coords = cur.fetchone()
        ...
        package_data = {
            ...
            "destination_name": package_row[18] if package_row[18] else "Delivery Location",
            "source_lat": drone_source_coords[0] if drone_source_coords else None,
            "source_lng": drone_source_coords[1] if drone_source_coords else None,
        }
