...
def update_drone_coordinates(conn, drone_id, destination_lat, destination_lng):
    """Updates the destination coordinates in the dronesdata table."""
    if not drone_id:
        return False
    
    try:
        with conn.cursor() as cur:
            # Update drone destination coordinates
            cur.execute("""
                UPDATE dronesdata 
                SET dest_lat = %s, dest_lng = %s 
                WHERE drone_id = %s;
            """, (destination_lat, destination_lng, drone_id))
            rows_affected = cur.rowcount
            app.logger.info(f"Updated destination coordinates for drone {drone_id}. Rows affected: {rows_affected}")
            return rows_affected > 0
    except psycopg2.Error as e:
        app.logger.error(f"Error updating drone coordinates: {e}")
        return False
...
def create_package():
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Update drone destination coordinates if assigned
            if data.get('assigned_drone_id'):
                update_drone_coordinates(conn, data.get('assigned_drone_id'), 
...
def update_package(package_id):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT assigned_drone_id, assigned_gripper, destination_lat, destination_lng FROM packagemanagement WHERE package_id = %s;", (package_id,))
            ...
            current_dest_lat = current_package['destination_lat']
            current_dest_lng = current_package['destination_lng']
    except psycopg2.Error as e:
    ...
    try:
        with conn.cursor() as cur:
            ...
            new_dest_lat = data.get('destination_lat')
            new_dest_lng = data.get('destination_lng')
            ...
            # Update drone destination coordinates if drone is assigned and coordinates changed
            drone_to_update = new_drone_id if new_drone_id is not None else current_drone_id
            dest_lat_to_update = new_dest_lat if new_dest_lat is not None else current_dest_lat
            dest_lng_to_update = new_dest_lng if new_dest_lng is not None else current_dest_lng
            
            if drone_to_update and ('destination_lat' in data or 'destination_lng' in data or 'assigned_drone_id' in data):
                update_drone_coordinates(conn, drone_to_update, dest_lat_to_update, dest_lng_to_update)
...
def delete_package(package_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            # Clear drone destination coordinates if drone was assigned
            if current_drone_id:
                update_drone_coordinates(conn, current_drone_id, None, None)
